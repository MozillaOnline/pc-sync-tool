...
function ImportFilesDialog(options) {
  this.initailize(options);
}

ImportFilesDialog.prototype = {
  initailize: function(options) {
    this.options = extend({
      onclose: emptyFunction
    }, options);
  ...
  closeAll: function() {
    var evt = document.createEvent('Event');
    evt.initEvent('ImportFilesDialog:show', true, true);
    document.dispatchEvent(evt);
  },
  ...
  _build: function() {
    ...
    document.addEventListener('ImportFilesDialog:show', this._onModalDialogShown);

    // Make sure other modal dialog has a chance to close itself.
    this._fireEvent('ImportFilesDialog:show');
...
/*   this._modalElement.addEventListener('mousedown', this.close.bind(this));
  ...
  close: function() {
    ...
    this._mask = null;
    this._modalElement = null;
    document.removeEventListener('ImportFilesDialog:show', this._onModalDialogShown);
    window.removeEventListener('resize', this._onWindowResize)
    this.options.onclose();
  }
};